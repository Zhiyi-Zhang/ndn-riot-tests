...
#define NDN_ACE_ENTRIES_NUMOF 20
...
typedef struct ndn_ace_entry{
    struct ndn_ace_entry* prev;
    struct ndn_ace_entry* next;
    ndn_block_t id;
    unsigned char seed[32]; 
}ndn_ace_entry_t;

static ndn_ace_entry_t _ace_table[NDN_ACE_ENTRIES_NUMOF];
...
{
    for (int i = 0; i < NDN_ACE_ENTRIES_NUMOF; ++i) {
                                     ...
                                     ndn_block_t* optional, ndn_block_t* dhbits)
{
    ...
    switch(snd){
        case ACE_CONSUMER_GLOBAL:
            ...
            ndn_shared_block_t* identity_name = ndn_name_move_from_comp(optional);

            ndn_ace_entry_t* entry = NULL;
            for (int j = 0; j < NDN_ACE_ENTRIES_NUMOF && _ace_table[j].id.buf ; ++j) {
                int r = ndn_name_compare_block(&_ace_table[j].id, &identity_name->block); 
                if(r == 0){
                    entry = &_ace_table[j];
                    break;
                }
...
static int on_interest(ndn_block_t* interest)
{
    ...
    ndn_shared_block_t* identity_name = ndn_name_move_from_comp(&comp);
    ...
    ndn_ace_entry_t* entry = NULL;
    for (int j = 0; j < NDN_ACE_ENTRIES_NUMOF; ++j) {
