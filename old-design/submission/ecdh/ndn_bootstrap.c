...
static int on_bootstrapping_response(ndn_block_t* interest, ndn_block_t* data)
{
    ...
    uint8_t buffer[64] = {0};
    memcpy(buffer, buf, 64); buf += 64; len -= 64;
    ...
    if (!uECC_shared_secret(buffer, ecc_key_pri, shared, curves[seq])) {
...
*/
    ndn_shared_block_t* sn2_new = ndn_name_append(&sn1->block, ecc_key_pub, 64); 
