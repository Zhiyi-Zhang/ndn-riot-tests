...
void test_ecdsa_signed_interest(ndn_name_t* name, ndn_name_t* identity)
{
  ...
  ndn_interest_tlv_encode_ecdsa_sign(&encoder, &interest, identity, &prv_key);
  ...
  int result = ndn_interest_tlv_decode_ecdsa_verify(&check_interest, pool, encoder.offset, &pub_key);
  if (result == 0) {
...
void test_hmac_signed_interest(ndn_name_t* name, ndn_name_t* identity)
{
  ...
  ndn_interest_tlv_encode_hmac_sign(&encoder, &interest, identity, &hmac_key);
  printf("interest block length: %d \n", (int) encoder.offset);
  printf("interest block content: \n");
  for (size_t i = 0; i < encoder.offset; i++) {
    printf("%d ", pool[i]);
  ...
  int result = ndn_interest_tlv_decode_hmac_verify(&check_interest, pool, encoder.offset, &hmac_key);
  if (result == 0) {
...
void test_digest_signed_interest(ndn_name_t* name)
{
  ...
  ndn_interest_tlv_encode_digest_sign(&encoder, &interest);
  printf("interest block length: %d \n", (int) encoder.offset);
  printf("interest block content: \n");
  for (size_t i = 0; i < encoder.offset; i++) {
    printf("%d ", pool[i]);
  ...
  int result = ndn_interest_tlv_decode_digest_verify(&check_interest, pool, encoder.offset);
  if (result == 0) {
